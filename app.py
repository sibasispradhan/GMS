  ...
  if request.method == 'POST':
    ...
    with engine.connect() as cur:
      ...
      if len(rows) > 0:
        ...
        #print('New Pwd', sha256_crypt.encrypt(new))
        #print('Pwd', password)
        #if sha256_crypt.verify(password_candidate, password):
        if password_candidate == new:
          ...
          if session['prof'] == 1:
            print('pwd2', password_candidate, new)
...
def deleteTrainor():
  ...
  with engine.connect() as cur:
    ...
    if request.method == 'POST':
      ...
      username = form.username.data
      q = cur.execute("SELECT username FROM trainors WHERE username != %s",
                      [username])
      b = cur.fetchall()
      new = b[0]['username']
      cur.execute("UPDATE members SET trainor = %s WHERE trainor = %s",
                  (new, username))
...
def addEquip():
  ...
  if request.method == 'POST' and form.validate():
    ...
    with engine.connect() as cur:
      ...
      if name in equips:
        cur.execute("UPDATE equip SET count = count+%s WHERE name = %s",
                    (count, name))
      else:
        cur.execute("INSERT INTO equip(name, count) VALUES(%s, %s)",
                    (name, count))
    #mysql.connection.commit()
    #cur.close()
...
def addMember():
  ...
  if request.method == 'POST' and form.validate():
    ...
    with engine.connect() as cur:
      ...
      cur.execute(
        "INSERT INTO members(username, plan, trainor) VALUES(%s, %s, %s)",
        (username, plan, trainor))
    #mysql.connection.commit()
    #cur.close()
    choices2.clear()
    choices.clear()
...
def viewDetails():
  with engine.connect() as cur:
    cur.execute("SELECT username FROM info WHERE username != %s",
                [session['username']])
    result = cur.fetchall()
...
def trainorDash():
  ...
  with engine.connect() as cur:
    ...
    cur.execute("SELECT username FROM members WHERE trainor = %s",
                [session['username']])
    members_under = cur.fetchall()
    #cur.close()
    #cur = mysql.connection.cursor()

    q = cur.execute("SELECT username FROM members WHERE trainor = %s",
                    [session['username']])
    b = cur.fetchall()
    for i in range(q):
      tup = (b[i]['username'], b[i]['username'])
  ...
  if request.method == 'POST':
    ...
    with engine.connect() as cur:
      p = cur.execute("SELECT date FROM progress WHERE username = %s",
                      [username])
      entered = []
      q = cur.fetchall()
      for i in range(p):
      ...
      if date in entered:
        cur.execute(
          "UPDATE progress SET daily_result = %s, rate = %s WHERE username = %s and date = %s",
      ...
      cur.execute(
        "INSERT INTO progress(username, date, daily_result, rate) VALUES(%s, %s, %s, %s)",
...
def updatePlans():
  ...
  if request.method == 'POST' and form.validate():
    ...
    with engine.connect() as cur:
      cur.execute(
        "SELECT name, exercise FROM plans WHERE name = %s and exercise = %s",
        (name, exercise))
      result = cur.fetchall()
      if len(result) > 0:
        cur.execute(
          "UPDATE plans SET sets=%s, reps= %s WHERE name = %s and exercise = %s",
          (sets, reps, name, exercise))
      else:
        cur.execute(
          "INSERT INTO plans(name, exercise, sets, reps) VALUES(%s, %s, %s, %s)",
...
def memberDash(username):
  ...
  with engine.connect() as cur:
    ...
    cur.execute("SELECT exercise, reps, sets FROM plans WHERE name = %s",
                [plan])
    scheme = cur.fetchall()
    n = cur.execute(
      "SELECT date, daily_result, rate FROM progress WHERE username = %s ORDER BY date DESC",
      [username])
...
def edit_profile(username):
  ...
  if request.method == 'POST' and form.validate():
    ...
    with engine.connect() as cur:
      ...
      q = cur.execute(
        "UPDATE info SET name = %s, street = %s, city = %s, phone = %s WHERE username = %s",
